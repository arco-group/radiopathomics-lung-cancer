...
def classifier(db, dataset_name, pat_filter, mod, os_lab, under_sampling, over_sampling, standardize, pca):
    t0 = time.time()
    ...
    for idx in id_patients.unique():
        ...
        if pca:
            ...
            print(f"Numero di feature: {transformer.n_components}")
...
def main():
    if platform.system() == "Windows":
        ...
        result_path = r"results/simple_late_fusion_DF_test.csv"
        ...
        result_path = "results/simple_late_fusion_DF_test.csv"
    ...
    df_path = df_path.sort_values(by = 0)
    df_rad = df_rad.loc[df_rad[0].isin(fuse_patients), :]
    df_rad = df_rad.sort_values(by = 0)
    df_sem = df_sem.loc[df_sem[0].isin(fuse_patients), :]
    df_sem = df_sem.sort_values(by = 0)


    df_patrad  = utils.kronecker(df_path, df_rad)
    df_patsem  = utils.kronecker(df_path, df_sem)
    df_radsem  = utils.kronecker(df_rad, df_sem)

    df_fin  = utils.kronecker(df_path, df_rad, df_sem)

    datasets = [df_path, df_rad, df_sem, df_fin, df_patsem, df_radsem, df_patrad]
    datasets_name = ["pathomics", "radiomic", "semantic", "pathomics_radiomic_semantic", "pathomics_semantic", "radiomics_semantic", "pathomics_radiomic"]

    pat_filter = []

    mods = [None]#, "balanced", "balanced_subsample"]
    for mod in mods:
        for dataset, dataset_name in zip(datasets, datasets_name):
            row = classifier(dataset, dataset_name=dataset_name, pat_filter=pat_filter, mod = mod, os_lab=False, under_sampling=False,
