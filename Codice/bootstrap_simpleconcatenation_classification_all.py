...
def classifier(db, dataset_name, result_path, pat_filter, os_lab, under_sampling, over_sampling, standardize, pca):
    ...
    if pat_filter:
        ...
        df_sem = df_sem.loc[~df_sem[0].isin(pat_filter), :]
    ...
    for j in range(n_bootstraps):
        ...
        for idx in id_patients.unique():
            ...
            X_test3 = df_sem.loc[df_sem[0] == idx, :]
            X_test3 = utils.aggregate(X_test3)

            X_test_fin = pd.merge(X_test1.iloc[:, 0:-1], X_test2, how='inner', on=0)
            X_test_fin.columns = range(len(X_test_fin.columns))
            X_test_fin = pd.merge(X_test_fin.iloc[:, 0:-1], X_test3, how='inner', on=0)
            X_test_fin.columns = range(len(X_test_fin.columns))

            X_test = X_test_fin.loc[:, 1:X_test_fin.shape[1] - 2]
            Y_test = X_test_fin.loc[:, X_test_fin.shape[1] - 1]

            df_path_bot = df_path.groupby(0).apply(
                lambda group_df: group_df.sample(frac=1 / (n_bootstraps - 1), replace=True)).reset_index(drop=True)
            df_path_bot = df_path_bot.drop_duplicates().reset_index(drop=True)
            df_path_bot = utils.aggregate(df_path_bot)

            df_rad_bot = df_rad.groupby(0).apply(lambda group_df: group_df.sample(frac=1 / (n_bootstraps - 1), replace=True)).reset_index(
                drop=True)
            df_rad_bot = df_rad_bot.drop_duplicates().reset_index(drop=True)
            df_rad_bot = utils.aggregate(df_rad_bot)

            df_sem_bot = df_sem.groupby(0).apply(lambda group_df: group_df.sample(n=10, replace=True)).reset_index(
                drop=True)
            df_sem_bot = df_sem_bot.drop_duplicates().reset_index(drop=True)
            df_sem_bot = utils.aggregate(df_sem_bot)

            df_patrad = pd.merge(df_path_bot.iloc[:, 0:-1], df_rad_bot, how='inner', on=0)
            df_patrad.columns = range(len(df_patrad.columns))
            df_patrad = pd.merge(df_patrad.iloc[:, 0:-1], df_sem_bot, how='inner', on=0)
            df_patrad.columns = range(len(df_patrad.columns))
