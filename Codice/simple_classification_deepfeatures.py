...
def main():
    if platform.system() == "Windows":
        deep_features = r"data/deep_features"
        #radiomics_path = r"data/radiomics.csv"
        #semantic_path = r"data/semantic-features.csv"
        result_path = r"results/simpleclassification_deepfeatures.csv"
    elif platform.system() == "Darwin":
        deep_features = "data/deep_features"
        #radiomics_path = "data/radiomics.csv"
        #semantic_path = "data/semantic-features.csv"
        result_path = "results/simpleclassification_deepfeatures.csv"

    pat_filter = []

    for dataset in os.listdir(deep_features):
        if not dataset.startswith("."):
            dataset_path = os.path.join(deep_features, dataset)

            df = pd.read_csv(dataset_path, header=None)
            df = utils.aggregate(df)

            row = classifier(df, dataset_name=dataset, pat_filter=pat_filter, os_lab=False, under_sampling=0.7,
                            over_sampling=False, standardize=True, pca=False)
