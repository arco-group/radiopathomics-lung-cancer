...
def main():
    if platform.system() == "Windows":
        ...
        radiomics_path = r"data/radiomics_handcrafted.csv"
        semantic_path = r"data/semantic-features.csv"
    elif platform.system() == "Darwin":
        pathomics_path = "data/pathomics.csv"
        radiomics_path = "data/radiomics_handcrafted.csv"
        semantic_path = "data/semantic-features.csv"
        result_path = "results/simpleconcatenation.csv"

    df_path = pd.read_csv(pathomics_path, header=None)
    df_path = utils.agregga_patch(df_path)
    df_path = df_path.rename(columns={df_path.columns[0]: 'ID_type'})

    df_rad = pd.read_csv(radiomics_path)
    df_rad = utils.aggrega_crop(df_rad)

    df_sem = pd.read_csv(semantic_path)
    df_sem = df_sem.rename(columns={df_sem.columns[0]: 'ID_type'})

    df_patrad = pd.merge(df_path.iloc[:, 0:-1], df_rad, how='inner', on='ID_type')
    df_patsem = pd.merge(df_path.iloc[:, 0:-1], df_sem, how='inner', on='ID_type')
    df_radsem = pd.merge(df_rad.iloc[:, 0:-1], df_sem, how='inner', on='ID_type')

    df_fin = pd.merge(df_patrad.iloc[:, 0:-1], df_sem, how='inner', on='ID_type')

    datasets = [df_path, df_rad, df_sem, df_patrad, df_patsem, df_radsem, df_fin]
    datasets_name = ["pathomics", "radiomic", "semantic", "pathomics_radiomic", "pathomics_semantic",
                     "radiomics_semantic", "pathomics_radiomic_semantic"]
