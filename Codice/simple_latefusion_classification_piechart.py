...
def classifier_loop(dbs, dataset_name, result_path, exp, pat_filter, os_lab, under_sampling, over_sampling, standardize,
                    pca):
    t0 = time.time()
    ...
    for idx in id_patients.unique():
        ...
        row = [exp, dataset_name, idx, y_true, y_pred]
...
def main():
    if platform.system() == "Windows":
        ...
        piechart_path = r"results/simple_late_fusion_piechart.csv"
        ...
        piechart_path = "results/simple_late_fusion_piechart.csv"
    ...
    i = 0
    row = ["exp", "dataset", "patient", "y_true", "y_pred"]
    utils.append_list_as_row(piechart_path, row)
    for dataset, dataset_name in zip(datasets, datasets_name):

        classifier_loop(dbs=dataset, dataset_name=dataset_name, exp = i, result_path=piechart_path, pat_filter=pat_filter,
