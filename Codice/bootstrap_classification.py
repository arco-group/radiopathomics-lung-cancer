...
def classifier(db, dataset_name, result_path, pat_filter, os_lab, under_sampling, over_sampling, standardize, pca):
    ...
    if pat_filter:
        db = db.loc[~db["ID_type"].isin(pat_filter), :]

    db.columns = range(db.shape[1])

    id_patients = db.iloc[:, 0]  # .to_list()
    ...
    for j in range(n_bootstraps):
        ...
        for idx in id_patients.unique():
            ...
            if min(db.groupby([0]).size()) != 1:
                db_bot = db.groupby(0).apply(
                    lambda group_df: group_df.sample(frac=0.5, replace=True)).reset_index(drop=True)
                db_bot = db_bot.drop_duplicates().reset_index(drop=True)
            else:
                db_bot = db.groupby(0).apply(
                    lambda group_df: group_df.sample(n=10, replace=True)).reset_index(
                    drop=True)
                db_bot = db_bot.drop_duplicates().reset_index(drop=True)
            ...
            ID_pat = db_bot.loc[db_bot[0] != idx, 0]
            X_train = db_bot.loc[db_bot[0] != idx, 1:db_bot.shape[1] - 2]
            Y_train = db_bot.loc[db_bot[0] != idx, db_bot.shape[1] - 1]
            Y_train_pat = db_bot.loc[db_bot[0] != idx, 0].tolist()
...
def main():
    ...
    df_sem = df_sem.rename(columns={df_sem.columns[0]: 'ID_type'})
    ...
    df_path = df_path.loc[df_path['ID_type'].isin(fuse_patients), :]
    df_rad = df_rad.loc[df_rad['ID_type'].isin(fuse_patients), :]
    df_sem = df_sem.loc[df_sem['ID_type'].isin(fuse_patients), :]
    ...
    for dataset, dataset_name in zip(datasets, datasets_name):
        classifier(dataset, dataset_name=dataset_name, result_path=result_path, pat_filter=pat_filter, os_lab=False,
